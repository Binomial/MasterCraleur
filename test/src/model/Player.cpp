...
std::string Player::findWords(std::string chevalet, s_pos anchor, int method) {
    std::string result = "!";
...
std::string Player::plays() {
    ...
    for (auto const& anchor : gameboard->getAnchors()) {
        result = findWords(letters, anchor.second, 0);
    }
